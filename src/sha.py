...
def sha1(file):
	...
	if os.path.exists(file):   #If the parameter is an existing file we hash it, otherwise we hash the string

		bit_string=''

		for b in bits(open(file, 'r')):       #read the bits of the file passed as arg one by one and concatenate them into a string
			bit_string+= str(b)
		...
		padded_data = message_pre_pro(file)
